...
RUN_DIR = BASE_DIR / "run_embed_onet_e5"
RUN_DIR.mkdir(parents=True, exist_ok=True)

CSV_PATH = BASE_DIR / "standard_df_onet_occupations_description_activities_and_tasks.csv"

# Updated to e5-large-v2
MODEL_NAME = os.environ.get("MODEL_NAME", "intfloat/e5-large-v2")
...
def main():
    ...
    # E5-v2 REQUIREMENT: Prepend "passage: "
    print("Prepending 'passage: ' prefix for E5...")
    roles_in = np.array(["passage: " + r for r in roles], dtype=object)
    tasks_in = np.array(["passage: " + t for t in tasks], dtype=object)
    ...
    for i in trange(start_idx, n, CHUNK_SIZE, desc=f"Embedding ONET chunks ({MODEL_TAG})"):
    ...
    # Sanity Diagnostics
    pairwise_cos_stats(role_embeds, f"ONET role_embeds ({MODEL_TAG})")
    pairwise_cos_stats(task_embeds, f"ONET task_embeds ({MODEL_TAG})")
