...
# Updated to the GTE model
MODEL_NAME = "Alibaba-NLP/gte-large-en-v1.5"

def shard_slice(n: int, shard_id: int, n_shards: int) -> slice:
    # contiguous split, last shard may be larger by 1
    base = n // n_shards
    rem = n % n_shards
...
def main():
    ...
    assert torch.cuda.is_available(), "No CUDA visible (are you on GPU node with proper env?)"

    data = np.load(in_npz, allow_pickle=True)
    job_ids = data["job_ids"].astype(str)
    roles = data["role_text"].astype(str)
    taskskills = data["taskskill_text"].astype(str)
    ...
    # GTE-Large-v1.5 requires trust_remote_code=True for its custom attention implementation
    dtype = torch.float16
