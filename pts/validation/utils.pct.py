...
def notebook_to_pdf(notebook_path: str | Path, output_dir: str | Path, stem: str) -> Path | None:
    """Convert an executed notebook to PDF via nbconvert (LaTeX), omitting code cells.

    Returns the PDF path, or None if conversion failed (e.g. LaTeX not installed).
    """
    import subprocess
...
def generate_reports_main():
    ...
    for nb_path in analyze_nbs:
        ...
        pdf_path = notebook_to_pdf(nb_path, reports_path, stem)
        if pdf_path:
            print(f"  PDF: {pdf_path}")
        else:
            print("  PDF: skipped (LaTeX not available or conversion failed)")
