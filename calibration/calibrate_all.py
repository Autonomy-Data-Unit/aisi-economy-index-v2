...
Usage:
    uv run python calibration/calibrate_all.py [--dry-run]

Reads all sbatch models from embed_models.toml and llm_models.toml, checks
which already have results in calibration/results/{llm,embed}/, and pairs
...
LLM_RESULTS_DIR = CALIBRATION_DIR / "results" / "llm"
EMBED_RESULTS_DIR = CALIBRATION_DIR / "results" / "embed"
CONFIG_DIR = CALIBRATION_DIR.parent / "config"
...
def main():
    ...
    all_llm = _get_sbatch_keys(CONFIG_DIR / "llm_models.toml")
    all_embed = _get_sbatch_keys(CONFIG_DIR / "embed_models.toml")
    ...
    for i, (llm, embed) in enumerate(pairs, 1):
        ...
        result = subprocess.run(
            [sys.executable, str(CALIBRATION_DIR / "run_calibration.py"), llm, embed],
        )
        if result.returncode != 0:
            print(f"\nERROR: run_calibration.py failed for {llm} + {embed} (exit code {result.returncode})")
            print("Stopping. Re-run calibrate_all.py to continue from where it left off.")
